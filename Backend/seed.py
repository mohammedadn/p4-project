...
from app import app, db, User, Workout, Trainer, Activity
...
def seed_data():
    ...
    with app.app_context():
        ...
        # Create workouts
        workout1 = Workout(name='Bench Press', email='john@example.com', address='123 Main St', contacts='John Doe', activity='Weightlifting', user_id=user1.id)
        workout2 = Workout(name='Yoga Class', email='jane@example.com', address='456 Oak Ave', contacts='Jane Smith', activity='Yoga', user_id=user2.id)
        ...
        for data in trainersData:
            trainer = Trainer(name=data['name'], speciality=data['specialty'], image=data['image'], email=data['email'], phone=data['phone'])
